 ...
 *
 * By default the built standalone CLI (`bin/run.js`) runs with `JIRA_CONFIG_DIR`
 ...
 * topic-prefixed (`sdkck jira issue PROJ-123`), so the argv is host-agnostic —
 * and oclif's bin-scoped `SDKCK_*` dirs are redirected: config to the same
 * throwaway `jira-config.json` dir the standalone leg uses, data/cache into the
 ...
 * @param configDir The dir holding jira-config.json, from createConfigDir().
...
function hostInvocation(
  ...
  return {argv: [CLI, ...args], command: process.execPath, env: {JIRA_CONFIG_DIR: configDir}}
}
 ...
 *
 * @param args Command line arguments, e.g. ['jira', 'project', 'list'].
 * @param configDir Value for JIRA_CONFIG_DIR / SDKCK_CONFIG_DIR, from
 *   createConfigDir().
